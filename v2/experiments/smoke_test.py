"""Minimal smoke test: one normal + one tamper test with Prover-Worker."""
...
try:
    ...
    print(f"Result: {s1} (expected: certified) {'PASS' if s1=='certified' else 'FAIL'}")

    # Test 2: Tamper last
    print("\n=== TEST: TAMPER LAST ===")
    r2 = run_distributed_pipeline(initial_input, artifacts, worker_urls, fault_at=2, fault_type="tamper")
    s2 = r2["certificate"]["status"]
    print(f"Result: {s2} (expected: invalid) {'PASS' if s2=='invalid' else 'FAIL'}")

    # Test 3: Skip
    print("\n=== TEST: SKIP LAST ===")
    r3 = run_distributed_pipeline(initial_input, artifacts, worker_urls, fault_at=2, fault_type="skip")
    s3 = r3["certificate"]["status"]
    print(f"Result: {s3} (expected: invalid) {'PASS' if s3=='invalid' else 'FAIL'}")
    ...
    print(f"\n{'='*40}")
    print(f"SMOKE TEST: {'ALL PASSED' if all_pass else 'SOME FAILED'}")
    for r in results:
